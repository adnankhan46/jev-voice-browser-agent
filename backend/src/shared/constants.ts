// @ts-nocheck
/**
 * ONE reviewable place for everything Laya sees and every threshold the policy uses.
 *
 * Laya (via Impossibl System One) does not generate text. Each request carries the
 ...
 *
 * Rules applied here (from the provider API contract):
 ...
 *  - always include a `none` option; never ask Laya to count or generate
 */

// export const MODEL = "convaiinnovations/laya";
// export const MODEL = "convaiinnovations/laya-multilingual";
export const MODEL = "typesafe-ai/jev";

export const PRICE_PER_M_INPUT_TOKENS_USD = 0.042; // Jev (from Impossibl)
// export const PRICE_PER_M_INPUT_TOKENS_USD = 0; // Laya (from Impossibl)

// ---------------------------------------------------------------------------
// Perception limits (state size hurts accuracy + latency; keep it small)
// ---------------------------------------------------------------------------
export const MAX_ELEMENTS = 100; // hard cap on elements sent to Laya (255 is the Choice limit; latency grows with tokens)
export const MAX_ELEMENT_TEXT = 60; // chars per element label
export const MAX_STATE_CHARS = 16_000; // reserve context for questions within Laya's 8k-token window
...
// last few executed actions (what was said, what was done, what happened). Lets Laya resolve
...
export const MAX_TASK_STEPS = 5; // limits autonomous browser actions for one spoken goal

// ---------------------------------------------------------------------------
// Timing
// ---------------------------------------------------------------------------
export const DEBOUNCE_MS = 200; // wait this long after the last transcript update before asking Laya // 300
export const MAX_INFLIGHT = 2; // overlapping Laya requests allowed; older ones are cancelled with AbortSignal
export const SILENCE_COMPLETE_MS = 700; // no new words for this long => treat command as complete //900
...
// ---------------------------------------------------------------------------
// Sites (code owns URLs; Laya only picks the name)
// ---------------------------------------------------------------------------
...
export const QUESTIONS = {
  ...
  task_status: {
    instructions: {
      question: "Given `task.goal`, the completed actions, and the current `page`, what should the browser agent do next?",
      focus: "Choose complete only when the user's goal has been achieved. Choose continue only when another browser action can make measurable progress. Choose clarify if the next step needs information, a login, or a human decision. Do not repeat an action already completed unless the page state proves it failed.",
    },
    criteria: {
      complete: "The requested goal is satisfied; stop acting.",
      continue: "The goal is not satisfied and a clear next browser action is possible.",
      clarify: "The next step is ambiguous, blocked, or needs user input/confirmation.",
    },
  },
};
