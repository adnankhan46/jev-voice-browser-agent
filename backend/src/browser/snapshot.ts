// @ts-nocheck
/**
 * Perception: turn the controlled page into a compact, model-friendly state.
 *
 ...
 *    text and guards the total state size so it stays far below Laya's 8k-token limit.
 */
import { MAX_ELEMENTS, MAX_ELEMENT_TEXT, MAX_STATE_CHARS } from "../shared/constants.ts";
...
/** Coarse site detection from the URL (code, not Laya). */
...
/**
 * Build the compact element list that goes into the Laya state.
...
/** Full snapshot record used by the controller: raw (for execution) + compact (for the model). */
